<#============================================================================
  File:     SQL Server Documentation.ps
  ...
  Prerequisites: 
  Both below modules need to be installed prior to documenting the 
  databases and servers

  1: Install-module ExcelPSLib
  2: (new-object Net.WebClient).DownloadString("https://github.com/brianbonk/SQLDoc/raw/master/Install SQLDoc module.ps1") | iex 

  Usage:
  Place a csv file in a folder of your own choice
    Headers:
    Computername;sqlserverinstancename

    Content:
    The servername/computername of the server; the SQL Server instace to be documented

  Change the folderpath in the bottom of the script and execute.
============================================================================#>

# Import the Excel library
Import-Module ExcelPSLib -Force
Import-Module SQLDoc -Force
...
function Generate-Documentation()
{
    ...
    [OfficeOpenXml.ExcelPackage]$excel = New-OOXMLPackage -author "ProActive" -title "Machine Information $target"
    ...
    if($data.OSArchitecture.Length -ge 1)
    {
        ...
        $sheet | Set-OOXMLRangeValue -row $rowPosition -col 1 -value "Free Virtual Memory MB" -StyleSheet $styleHeader2 | Out-Null
    ...
    # Add a worksheet
    $excel | Add-OOXMLWorksheet -WorkSheetName "Instance Information" #-AutofilterRange "A2:G2"
    # Set the worksheet as the first sheet
    $sheet = $book | Select-OOXMLWorkSheet -WorkSheetNumber $tabPosition

    Write-Host " - Retrieving data for Instance information" -ForegroundColor Green

    $configurations = Get-ServerProperties -inst $target | Sort DisplayName

    if($configurations.Count -ge 1)
    {

        # Create the header
        $sheet | Set-OOXMLRangeValue -row $RowPosition -col 1 -value "Information Name" -StyleSheet $StyleHeader2 | Out-Null
        $sheet.Column(1).Width = 40
        $sheet | Set-OOXMLRangeValue -row $RowPosition -col 2 -value "Information Value" -StyleSheet $StyleHeader2 | Out-Null
        $sheet.Column(2).Width = 32
        ...
        foreach($config in $configurations)
        {
            ...
            $sheet | Set-OOXMLRangeValue -row $rowPosition -col 1 -value $config.Value -StyleSheet $styleNormal | Out-Null
            $sheet | Set-OOXMLRangeValue -row $rowPosition -col 2 -value $config.Data -StyleSheet $styleNormal | Out-Null
    ...
    {
        Write-Host "No records fount for Instance Information" -ForegroundColor Red
    }
...
$items = Import-Csv "C:\Users\brbo\ProActive\Nationalbanken BI konsulentbistand - Project documents\2 - Solution Documentation\SOW 001 - PBI - on premise\Documentation\sqlserverinstances.csv" -Delimiter ";"

foreach($item in $items)
{
    if(Test-Connection $item.ComputerName)
    {
        Generate-Documentation -server $item.ComputerName -instance $item.SQLServerInstanceName -destination 'C:\Users\brbo\ProActive\Nationalbanken BI konsulentbistand - Project documents\2 - Solution Documentation\SOW 001 - PBI - on premise\Documentation\'
    }
}
